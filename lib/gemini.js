...
class GeminiService {
   ...
   * @param {number} maxWords - Target words in summary (default: 1400)
   */
  async generateSummaryFromPDF(filePath, maxWords = 1400) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const prompt = `
You are an expert study assistant. Read this ENTIRE document page by page and create a comprehensive, detailed summary.

**CRITICAL — Do NOT be concise:**
- Your summary should be AS LONG AS POSSIBLE while remaining well-organized. Aim for at least ${maxWords} words. More is better.
- Cover every section, every concept, every definition, every example in the document.
- Do not skip or compress any content. Write as if this is the only record the reader will have.
- Use headings (## Section Title), bullet points, and paragraphs to organize.
- Preserve all technical terms, names, dates, and definitions exactly.
- Make it suitable for audio — use complete sentences and natural transitions.
