...
class GeminiService {
  ...
  /**
   * Generate summary from image-based PDF using Gemini Vision
   * @param {string} filePath - Path to PDF file
   * @param {number} maxWords - Maximum words in summary (default: 1400)
   */
  async generateSummaryFromPDF(filePath, maxWords = 1400) {
    let lastError;
    
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        logger.info(`Generating summary from image PDF (attempt ${attempt}/${this.maxRetries})...`);
        ...
        const prompt = `
You are an expert study assistant. Your task is to read this document carefully and create a highly detailed, comprehensive summary.

**Critical Instructions:**
1. Your summary MUST be approximately ${maxWords} words. Do not write a short summary. Be thorough.
2. Read every page. Do not skip any content.
3. Cover ALL key concepts, main ideas, definitions, examples, and important details from the entire document.
4. Organize the summary in a clear, logical structure with sections and bullet points.
5. Preserve technical terms, definitions, formulas, and important names exactly as they appear.
6. If the document contains diagrams, charts, or tables, describe their key insights in detail.
7. Write as if the reader has never seen the original document — leave nothing important out.
8. Make it suitable for listening as audio — use complete sentences and clear transitions.

**Summary (aim for ${maxWords} words):**
`;
        ...
        logger.info(`Summary from image PDF generated successfully: ${wordCount} words`);
        
        return summary;
      } catch (error) {
        lastError = error;
        logger.error(`Image PDF summary attempt ${attempt} failed:`, error.message);
    ...
    throw new Error(`Failed to generate summary from image PDF after ${this.maxRetries} attempts: ${lastError.message}`);
  }
...
7. Your summary should not exceed ${maxWords} words
