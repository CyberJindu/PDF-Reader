  ...
  async generateSummaryFromPDF(filePath, maxWords = 1400) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const prompt = `
You are an expert study assistant. Your task is to read and summarize the content of this document.

This document is an image-based/scanned PDF. Read all visible text from every page carefully.
...
7. If the document contains diagrams or charts, describe their key insights in text.
8. Your summary should not exceed ${maxWords} words.
