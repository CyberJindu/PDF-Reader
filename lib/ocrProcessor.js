const PdfToImage = require('pdf-image');
...
async function extractTextFromPDF(pdfPath, options = {}) {
  ...
  try {
    ...
    // Convert PDF to images using pdf-to-image (no external dependencies)
    const pdf = new PdfToImage(pdfPath);
    
    // Convert all pages to images
    const pageResults = await pdf.convertPages({
      outputDir: savePath,
      quality: quality,
      density: density,
      width: 1024,
    ...
    for (let batchStart = 0; batchStart < pageCount; batchStart += parallelPages) {
      const batchEnd = Math.min(batchStart + parallelPages, pageCount);
      const batchPages = [];
      
      // Get batch pages from the results
      for (let i = batchStart; i < batchEnd; i++) {
        ...
        try {
          ...
          const imageStats = await fs.promises.stat(page.path);
          ...
          logger.error(`Error validating page ${pageNumber}:`, convertError);
          pageTexts.push({
            page: pageNumber,
            text: '',
            error: `Validation failed: ${convertError.message}`,
            confidence: 0
          });
        }
      }
