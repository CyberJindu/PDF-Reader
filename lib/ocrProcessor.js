...
async function validatePDF(pdfPath) {
  try {
    ...
    await testConvert.getPageCount();
    return true;
  } catch (error) {
...
async function extractTextFromPDF(pdfPath, options = {}) {
  const {
    ...
    maxRetries = 3,
    timeout = 30000,
    parallelPages = 3, // Reduced from 5 to prevent overload
    allowedDirectories = [] // Allow specific directories
  } = options;
  ...
  try {
    ...
    // Allow both uploads and temp directories
    const isAllowedPath = 
      resolvedPath.startsWith(uploadsDir) || 
      resolvedPath.startsWith(tempDir) ||
      allowedDirectories.some(dir => resolvedPath.startsWith(path.resolve(dir)));
    
    if (!isAllowedPath) {
      throw new Error('Invalid file path - file must be in uploads or temp directory');
    }
